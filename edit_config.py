...
def edit(train_dir, eval_dir, batch_size, checkpoint, label_map):
    ...
    reader = pipeline_config.train_input_reader
    reader.label_map_path = label_map
    reader.tf_record_input_reader.input_path[:] = training_files

    reader_ = pipeline_config.eval_input_reader[0]
    reader_.label_map_path = label_map
    reader_.tf_record_input_reader.input_path[:] = evaluation_files
